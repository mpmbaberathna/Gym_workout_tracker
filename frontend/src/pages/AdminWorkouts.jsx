    ...
    <div className="relative min-h-screen w-full bg-brand-light text-brand-dark px-8 md:px-16 pt-28 pb-20">
      <div className="relative z-10 mx-auto max-w-6xl space-y-12">
        <div className="text-center mb-10">
          ...
          <h1 className="text-4xl font-extrabold tracking-tight text-brand-dark sm:text-5xl">All Workouts</h1>
          <p className="mt-4 text-base text-brand-gray leading-relaxed">Browse and review workouts created across the platform.</p>
        </div>
          ...
          {workouts.length === 0 && !error ? (
            ...
            <div className="grid gap-6">
              {workouts.map((workout) => (
                <div key={workout._id} className="group relative flex flex-col overflow-hidden rounded-3xl border border-brand-border bg-brand-card p-6 shadow-sm transition hover:shadow-md hover:border-brand-dark/20">
                  <div className="flex flex-col md:flex-row items-start justify-between gap-6">
                    <div className="flex-1">
                      <p className="text-xl font-bold text-brand-dark">{workout.title}</p>
                      {workout.createdBy && (
                        <p className="mt-2 text-xs font-bold uppercase tracking-wider text-brand-gray">
                          Owner: {workout.createdBy.name || workout.createdBy.email} {workout.createdAt ? `• ${new Date(workout.createdAt).toLocaleString()}` : ''}
                        </p>
                      )}
                    </div>
